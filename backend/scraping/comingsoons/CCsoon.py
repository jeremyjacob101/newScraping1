    ...
    def logic(self):
        ...
        for cinema_block in range(1, self.lenElements("#moviesContainer > div", "row mainThumbWrapper") + 1):
            for film_card in range(1, self.lenElements(f"/html/body/div[4]/div/div/div/div[1]/div[2]/div/div[{cinema_block}]/div") + 1):
                ...
                if "מדובב לרוסית" in self.hebrew_title or "בתרגום לרוסית" in self.hebrew_title or "מדובב לצרפתית" in self.hebrew_title or "בתרגום לצרפתית" in self.hebrew_title or "סינמה קידס" in self.hebrew_title or "cook" in self.hebrew_title or "מדובב" in self.hebrew_title or "HFR" in self.hebrew_title:
                    continue
